...
const UserFollowerContainer = ({item, onFollower,authId,onNavigate,authStateFollowing,authState}) => {
    ...
    <View style={{width: '100%'}} activeOpacity={0.6}>
      <View style={commonStyles.rowJustify}>
        ...
        {authId==item?.uid?(
            ...
            borderRadius={8}
            onPress={onFollower}
            backgroundColor={item?.RequestIds?.includes(authState.uid)?colors.green:authStateFollowing?.includes(item.uid)?colors.lightGray:colors.green}
            color={colors.white}
            fontSize={13}
            title={item?.RequestIds?.includes(authState.uid)?"Requested": authStateFollowing?.includes(item.uid)?"Following": 'Follow'}
            width={'27%'}
          />
        )
        }
