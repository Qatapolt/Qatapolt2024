...
const ChatListContainer = props => {
  ...
  const getUser = async () => {
    const data = await getSpecificUser(props.userId);
    setUser(data);
  };
  ...
  return (
    <TouchableOpacity
      ...
      <View style={{flexDirection: 'row', alignItems: 'center', width: '70%'}}>
        <CustomImage width={45} height={45} imageUrl={user?.profileImage} />
    ...
    //   onPress={() => props.navigation.navigate('ChatDetail',{userId:props.userId,formId:props?.formId})}>
    //   <ListItem >
