...
const OtherUserProfile = ({ navigation, route }) => {
  ...
  useFocusEffect(
    React.useCallback(() => {
      const fetchData = async () => {
        try {
          console.log("running focus otherUserProfile", isLoading);
          setIsLoading(true);
          ...
          getAllPost(),
            setTimeout(() => {
              fetchUserPosts(), fetchHighlights(), fetchSquad();
            }, 500);
  ...
  const BlockUser = async () => {
    try {
      await UpdateBlockUsers(authUser.uid, userData?.uid);
      // await UpdateBlockUsersOthers(userData?.uid, authUser.uid);
